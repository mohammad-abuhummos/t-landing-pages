...
const LEADPROSPER_CONFIG = {
    campaignId: process.env.LEADPROSPER_CAMPAIGN_ID ?? '31308',
    supplierId: process.env.LEADPROSPER_SUPPLIER_ID ?? '97682',
    key: process.env.LEADPROSPER_KEY ?? 'z6nzcnx16slx5v',
    action: process.env.LEADPROSPER_ACTION ?? '',
    defaultSubId: process.env.LEADPROSPER_DEFAULT_SUBID ?? 'default',
};

const ALLOWED_PROJECT_TYPES = new Set(['Shower', 'Bath', 'Full Bathroom Remodeling']);
...
const mapProjectType = (projectType: string) => {
    ...
    if (normalized.includes('full')) return 'Full Bathroom Remodeling';
    return 'Full Bathroom Remodeling';
};
...
export async function POST(request: NextRequest) {
    try {
        ...
        const payload: Record<string, string> = {
            ...
            lp_supplier_id: LEADPROSPER_CONFIG.supplierId,
            lp_key: LEADPROSPER_CONFIG.key,
            lp_subid1: lpSubId1,
        ...
        console.error('LeadProsper bathroom submission failed:', error);
        return NextResponse.json(
...
export async function GET() {
    return NextResponse.json({
        message: 'Bathroom LeadProsper endpoint is online',
