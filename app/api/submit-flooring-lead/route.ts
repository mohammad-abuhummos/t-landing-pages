...
import { readJsonFromDataOrRepo } from '@/helpers/dataPaths';

export async function POST(request: NextRequest) {
    try {
        const body = await request.json();

        // Validate required fields based on flooring API documentation
        const requiredFields = [
            'firstName', 'lastName', 'phone', 'email', 'zipCode', 'state',
            'homeOwnership', 'projectType', 'floorType'
        ];
        ...
        // Prepare trafficom.co API data for flooring (TYPE 39)
        console.log('Client IP detected:', clientIP);
        // Resolve SRC from s1 mapping (flooring vertical), fallback to env
        const s1Value = (leadData.s1 || '').toString();
        const runtimeSrcMap = await readJsonFromDataOrRepo<Record<string, any>>("config/srcMap.json", {});
        const mappedSrc = (runtimeSrcMap as any).flooring?.[s1Value] || (runtimeSrcMap as any).global?.[s1Value] || process.env.LEAD_API_SRC;
        const leadApiData = {
            Request: {
...
export async function GET() {
    return NextResponse.json({
        message: 'Flooring lead submission endpoint is working',
