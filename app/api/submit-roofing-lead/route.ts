...
export async function POST(request: NextRequest) {
    try {
        ...
        const requiredFields = [
            ...
            'homeOwnership', 'projectType', 'propertyType', 'roofType'
        ];
        ...
        console.log('Client IP detected:', clientIP);
        // Resolve SRC from s1 mapping (roofing vertical), fallback to env
        ...
        const mappedSrc = (runtimeSrcMap as any).roofing?.[s1Value] || (runtimeSrcMap as any).global?.[s1Value] || process.env.LEAD_API_SRC;
        const leadApiData = {
            Request: {
